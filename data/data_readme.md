...
In this tiny template, the pull step uses the built-in `mtcars` dataset from `plotnine`, so `data/external/` is empty by default. The folder is still present so that students can later adapt the structure to their own projects.
